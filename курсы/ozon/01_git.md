...
- https://stackoverflow.com/questions/57265785/whats-the-difference-between-git-switch-and-git-checkout-branch
	...
	- ==**`git checkout`**== — это оригинальная команда, которая использовалась как для переключения между ветками, так и для восстановления файлов (что могло запутать новичков).
    
	- ==**`git switch`**== и ==**`git restore`**== были добавлены в Git 2.23 (2019) для разделения этих двух задач:
    
    - ==`git switch`== — только для переключения веток.
        
    - ==`git restore`== — для восстановления файлов (замена `git checkout -- <file>`).
	
	### 2. **Переключение между ветками**
	
	- ==**`git switch <branch>`**== — переключается на существующую ветку `branch`.
    
    - Пример: `git switch main`.
        
    - Создание новой ветки: `git switch -c <new-branch>` (аналог `git checkout -b`).
        
	- **`git checkout <branch>`** — делает то же самое, но имеет «перегруженный» функционал.
> Обе команды работают для переключения веток, но `git switch` более специализированная и понятная.
