...
---
расписать незнакомые команды

1. Повседневная работа с командной строкой (CLI)
- **Базовые команды** (==`ls`==, ==`cd`==, ==`mkdir`==, ==`rm`==, ==`cp`==, ==`mv`==, ==`sudo`== и т.д.).
    - _Пример задачи:_ “Нужно быстро создать пару директорий, скопировать конфиги, дать правильные права. Без этих команд вы никуда.”
- **Фильтрация и поиск** (==`grep`==, ==`find`==, ==`xargs`==, ==`sort`==, ==`awk`==, ==`sed`==).
    - _Пример задачи:_ “Логи разрослись на 10 тысяч строк, нужно найти только ошибки за последнюю минуту.”
- **Понимание структуры каталогов** (`/etc`, `/var/log`, `/usr`, `/home` и т.д.).
    - _Пример задачи:_ “Вы знаете, что все конфиги лежат чаще всего в `/etc`, логи — в `/var/log`. Если нужно поправить сервис, вы двигаетесь именно туда.”
...
- Что такое ==`chmod`==, ==`chown`==, ==`umask`==, и как управлять правами в Linux.
    - _Пример задачи:_ “Хотите, чтобы разработчики могли работать с рабочей директории, но не читать чужие логи? Нужно правильно настроить группы и права.”


3. Управление сервисами (Systemd, SysV init)
- **Systemd**: сервисные юниты (`.service`), логи (`journalctl`), команды (`systemctl start/stop/restart`).
- _Пример задачи:_ “Нужно, чтобы ваш Nginx поднимался автоматически после перезагрузки. Вы создаёте сервис-юнит, добавляете `WantedBy=multi-user.target`, проверяете, всё ли ок.”


4. Процессы и ресурсы
- Понимать, как посмотреть, какие процессы занимают CPU, RAM (==`ps`==, ==`top`==, ==`htop`==).
- _Пример задачи:_ “На проде 100% CPU, DevOps быстро смотрит `top`, замечает подозрительный процесс, решает, что нужно убить его либо перезапустить этот сервис.”

5. Пакетные менеджеры
- **apt**, **yum**, **dnf**, **zypper** (зависит от дистрибутива).
- _Пример задачи:_ “Вам нужно поставить `git` или `docker` на Ubuntu — `apt-get install docker.io` (с соответствующими репозиториями). Уметь подключать репозиторий, обновлять кеш.”

6. Логи и журналы
- `/var/log/syslog`, `/var/log/messages`, `/var/log/nginx/error.log`, `/var/log/journal/...`
- _Пример задачи:_ “Приложение упало, DevOps сразу идёт в `/var/log`, смотрит `syslog` или журнал конкретного сервиса, находит ошибку.”

- `nslookup`, `dig`: проверяют, куда “смотрит” домен.

**iptables / nftables / firewalld**


Полезные утилиты “из коробки”
1. `ping`
...
3. `telnet` или `nc (netcat)`
    - Проверить доступ к порту: “telnet 1.2.3.4 80” — если получилось, значит порт открыт, сервис отвечает.
    - netcat (`nc`) тоже пробует соединиться или слушать порт, бывает очень удобно.
4. `nslookup` / `dig`
    - Для проверки DNS-записей (A, MX, CNAME).